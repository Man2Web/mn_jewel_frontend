...
export function Header() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prevIndex) => (prevIndex + 1) % materialPrice.length)
    }, 3000) // Change text every 3 seconds
    return () => clearInterval(interval)
  }, [materialPrice.length])
    ...
    <div>
      <header className="flex w-full items-center border md:px-4 md:py-2 lg:justify-between">
        ...
        <div className="ml-auto flex items-center space-x-4 px-4 lg:ml-0">
          <div className="flex cursor-pointer items-center gap-4 rounded-full border border-red-200 bg-red-50 px-4 py-2 hover:border-red-400 hover:bg-red-100">
            <svg
            ...
            {materialPrice.length > 0 && (
              <div className="flex animate-fade-in-out gap-2 text-sm font-semibold opacity-50">
                <p>{`${materialPrice[currentIndex].name}:`}</p>
                <p>{Number(materialPrice[currentIndex].price).toLocaleString()}</p>
              </div>
            )}
          </div>
          <div>
